...
class Shift(models.Model):
    ...
    description = models.CharField(max_length=30) # sample `description`: "MON 12:00 - 13:30"
    day = models.CharField(max_length=10, choices=DAYS) # `day` has to be within the weekdays
    start = models.TimeField(auto_now=False, auto_now_add=False) # `start` is a `datetime.datetime.time` object
    end = models.TimeField(auto_now=False, auto_now_add=False) # `end` is a `datetime.datetime.time` object
...
class UTA(models.Model):
    ...
    """
    fullname = models.CharField(max_length=100, default="") # UTA fullname (e.g. Hugh Mann)
    emplid = models.CharField(max_length=8, validators=[MinLengthValidator(8)]) # UTA's 8 digit unique CUNY empl ID
    shifts = models.ManyToManyField(Shift) # reference to multiple `Shift` objects that the UTA is assigned to work in
...
class Checkin(models.Model):
    ...
    emplid = models.CharField(max_length=8, validators=[MinLengthValidator(8)]) # UTA's 8 digit unique CUNY empl ID
    shift = models.ForeignKey(Shift, on_delete=models.CASCADE) # allows many `Checkin` objects to reference single `Shift` object 
    late_mins = models.IntegerField(default=0) # stores how late the checkin is from the `shift` start time
    number_of_shifts = models.IntegerField(default=1) # allows checkin for consecutive shifts
    ...
    alternate_day = models.CharField(max_length=10, choices=DAYS, default="") # allows checkin to another day of the week if CUNY is following alternate day schedule
